...
def benchmark_method(method,device='cuda',dtype='float32',batch_size=16,is_weight_decay=False):
    if dtype=='float32':
        dtype=torch.float32
    elif dtype=='float16':
        dtype=torch.float16
    seqlens = [128,512,2048,8192,12800,25600,100000]
    ans = {
        'seqlen': seqlens,
        'time':[],
    }
    for n in seqlens:
        t = 'OOM'
        try:
            B = torch.randn(batch_size,32,n,128,dtype=dtype,device=device)
            C = torch.randn(batch_size,32,n,128,dtype=dtype,device=device)
            V = torch.randn(batch_size,32,n,128,dtype=dtype,device=device)
            if is_weight_decay:
                s = _build_slope_tensor(h).to(dtype=type,device=device).reshape(h)
            ...
            print(f'There is some error in your code: {e}')
        finally:
            ans['time'].append(t)
    df = pd.DataFrame(ans)
    print('Performance:\n')
